...
from src.train_eval import loss_function, test
...
def attention_weights(config_params, model, dataloader_test, batches_test, df_test, path_to_output):
    ...
    with torch.no_grad():
        for test_idx, test_batch, test_labels, views_names, _ in dataloader_test:
            ...
            for id_case in test_idx:
                ...
                if df_image_case['Views'].tolist()!= views_names:
                    input('halt')
                image_label_percase.append(df_image_case['ImageLabel'].CNN(groundtruth_dic).tolist()) 
                if len(np.unique(df_image_case['ImageLabel'].CNN(groundtruth_dic)))>1:
                    print(batch_test_no)
                    print(id_case)
                    print(df_image_case)
                    flag = 1
            ...
            if config_params['femodel'] == 'gmic_resnet18':
                if config_params['learningtype'] == 'SIL':
                    ...
                    output_batch_local, output_batch_global, output_batch_fusion, saliency_map = model(test_batch, views_names, eval_mode)
