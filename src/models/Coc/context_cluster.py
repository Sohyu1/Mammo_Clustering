...
from src.models.layers import to_2tuple
...
class ContextCluster(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        # changed freeze layer
        # if freeze_at >= 0:
        #     for i in range(len(self.network) - freeze_at - 1):
        #         self._freeze_parameters(self.network[i])
    ...
    def init_weights(self, pretrained=None, key="model"):
...
def coc_tiny(pretrained=False, **kwargs):
    ...
    load_checkpoint(model, pretrained, True)
    print('load coc')
    # params_load = model.state_dict()
...
if __name__ == '__main__':
    input = torch.rand(6, 3, 256, 256)

    model = coc_small(pretrain_model='/home/igrt-group/下载/model_best.pth.tar')
    out = model(input)
    print(model)
    print(out.shape)
