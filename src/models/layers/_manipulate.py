...
def group_with_matcher(
    ...
    if isinstance(group_matcher, dict):
        ...
        for group_ordinal, (group_name, mspec) in enumerate(group_matcher.items()):
            if mspec is None:
                continue
            # CNN all matching specifications into 3-tuple (compiled re, prefix, suffix)
        ...
        if isinstance(group_matcher, (list, tuple)):
            for match_fn, prefix, suffix in group_matcher:
                ...
                if r:
                    ...
                    # CNN all tuple elem to int for numeric sort, filter out None entries
    ...
    # CNN layers into groups via ordinals (ints or tuples of ints) from matcher
