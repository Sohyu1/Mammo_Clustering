...
from src.utilities import utils, data_augmentation_utils, data_loaders_utils
...
def data_augmentation(config_params, df_train):
    ...
    if config_params['dataaug'] == 'gmic':
        ...
        preprocess_val = data_augmentation_utils.data_augmentation_train_shen_gmic(config_params, mean, std_dev)
...
def dataloader(config_params, df_train, df_val, df_test, view_group_indices_train, g):
    ...
    if config_params['viewsinclusion'] == 'all':
        if config_params['classimbalance'] == 'oversampling':
            ...
            sampler = data_loaders_utils.CustomGroupbyViewFullRandomSampler(view_group_indices_train,
                                                                            config_params['batchsize'], 'train')
            sampler_val = data_loaders_utils.CustomGroupbyViewRandomSampler(df_val, 'val')
        ...
        batch_sampler = data_loaders_utils.CustomGroupbyViewFullRandomBatchSampler(sampler, config_params['batchsize'],
                                                                                   view_group_length)
        batch_sampler_val = data_loaders_utils.CustomGroupbyViewRandomBatchSampler(sampler_val,
                                                                                   config_params['batchsize'],
                                                                                   view_group_length_val,
                                                                                   view_group_name_val)
        batch_sampler_test = data_loaders_utils.CustomGroupbyViewRandomBatchSampler(sampler_test,
                                                                                    config_params['batchsize'],
                                                                                    view_group_length_test,
                                                                                    view_group_name_test)
        batch_size1 = 1
        shuffle = False
    ...
    if config_params['learningtype'] == 'SIL':
        ...
        dataloader_train = DataLoader(dataset_gen_train, batch_size=batch_size1, shuffle=shuffle,
                                      num_workers=config_params['numworkers'], collate_fn=data_loaders_utils.MyCollate,
                                      worker_init_fn=seed_worker, generator=g, sampler=sampler1,
        ...
        if config_params['usevalidation']:
            ...
            dataloader_val = DataLoader(dataset_gen_val, batch_size=batch_size1, shuffle=False,
                                        num_workers=config_params['numworkers'],
                                        collate_fn=data_loaders_utils.MyCollate, worker_init_fn=seed_worker,
                                        generator=g, batch_sampler=batch_sampler_val)

        dataset_gen_test = data_loaders_utils.BreastCancerDataset_generator(config_params, df_test, preprocess_val)
        dataloader_test = DataLoader(dataset_gen_test, batch_size=batch_size1, shuffle=False,
                                     num_workers=config_params['numworkers'], collate_fn=data_loaders_utils.MyCollate,
                                     worker_init_fn=seed_worker, generator=g, batch_sampler=batch_sampler_test)

    elif config_params['learningtype'] == 'MIL' or config_params['learningtype'] == 'MV' or config_params['learningtype'] == 'DS':
        dataset_gen_train = data_loaders_utils.BreastCancerDataset_generator(config_params, df_train, preprocess_train)
        dataloader_train = DataLoader(dataset_gen_train, batch_size=batch_size1, shuffle=shuffle,
                                      num_workers=config_params['numworkers'],
                                      collate_fn=data_loaders_utils.MyCollateBreastWise, worker_init_fn=seed_worker,
                                      generator=g, sampler=sampler1, batch_sampler=batch_sampler)
        ...
        dataloader_test = DataLoader(dataset_gen_test, batch_size=batch_size1, shuffle=False,
                                     num_workers=config_params['numworkers'],
                                     collate_fn=data_loaders_utils.MyCollateBreastWise, worker_init_fn=seed_worker,
                                     generator=g, batch_sampler=batch_sampler_test)
