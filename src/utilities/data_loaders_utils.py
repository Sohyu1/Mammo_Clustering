...
from src.utilities import utils, data_augmentation_utils, gmic_utils
...
class BreastCancerDataset_generator(Dataset):
    ...
    def __getitem__(self, idx):
        if self.config_params['learningtype'] == 'SIL':
            ...
            flag = 0
            data = self.df.loc[
                idx]  # loc is valid because I have reset_index in df_train, df_val and df_test. random data sampling returns index, but as our index is same as the relative position, bpth iloc and loc should work. I am using loc because of groupby view data sampler.
            image_list, _, views_saved, studyuid_path = collect_cases(self.config_params, data)
            ...
            return idx, image_tensor, torch.tensor(self.config_params['groundtruthdic'][data['Groundtruth']]), views_saved, studyuid_path
        elif self.config_params['learningtype'] == 'DS':
            flag = 0
            ...
            for j, img in enumerate(image_list):
                ...
                if flag == 0:
                    ...
                    image_tensor = torch.cat((image_tensor, img), 0)
            return idx, image_tensor, torch.tensor(
                self.config_params['groundtruthdic'][data['Groundtruth']]), views_saved, studyuid_path
    ...
    def __iter__(self):
        ...
        for i in range(0, 2):
            if i != max_class[0]:
                ...
                OriginalIndices = np.append(OriginalIndices, np.random.choice(np.where(labels == i)[0], leftover),
                                            axis=0)
...
class CustomGroupbyViewRandomBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        for idx in self.sampler:
            ...
            if len(batch) == self.batch_size or len_tillCurrent == self.view_group_length_dic[
                self.view_group_name[file_current]]:
...
def view_extraction(series_list, views_allowed):
    series_list = list(map(lambda x: [x.split('.')[0].replace(' ', '').upper(), x], series_list))
...
def collect_cases(config_params, data):
    ...
    series_list = [element + '.png' for element in data['Views'].split('+')]
    if series_list[0].split('.')[-1] == 'png':
        for series in series_list:
            img_path = studyuid_path + '/' + series
            data1['FullPath'] = img_path
            data1['Views'] = series.split('.')[0]
            img = collect_images(config_params, data1)
            if series.split('.')[0] in views_allowed and series.split('.')[0] not in views_saved:
                views_saved.append(series.split('.')[0])
                image_read_list.append(img)
                breast_side.append(series.split('.')[0][0])
    else:
        for series in series_list:
            ...
            for image in img_list:
                ...
                if series[0] in views_allowed and series[0] not in views_saved:
                    if config_params['dataset'] == 'zgt' and config_params['viewsinclusion'] == 'all' and config_params[
                        'bitdepth'] == 12:  # solve this in future
                        ...
                        image_read_list.append(img)
                        breast_side.append(series[0][0])
    return image_read_list, breast_side, views_saved, studyuid_path
...
def collect_images_gmiccleaningcode(data):
    breast_side = data['Views'][0]
    loaded_image = gmic_utils.load_image(image_path=data['FullPath'], view=data['Views'], horizontal_flip='NO',
...
def MyCollateBreastWise(batch):
    ...
    if batch[0][1].shape[1] == 3:
        data = torch.zeros(len(batch), len(batch[0][3]), batch[0][1].shape[1], batch[0][1].shape[2],
                           batch[0][1].shape[3])
    ...
    index = torch.LongTensor(index)
    target = torch.LongTensor(target)
    return [index, data, target, list(np.unique(views_names_list)), path]
