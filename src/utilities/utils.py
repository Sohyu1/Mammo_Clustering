...
from src.utilities import data_loaders_utils
...
def class_distribution_weightedloss(config_params, df):
    ...
    class_weight = utils.class_weight.compute_class_weight(class_weight='balanced',
                                                           classes=np.array(config_params['classes']), y=df_groundtruth)
...
def stratifiedgroupsplit(df, rand_seed):
    ...
    for group_id, group in groups:
        ...
        train_inds, val_inds = next(
            train_valsplit.split(group.iloc[train_inds1], groups=group.iloc[train_inds1]['Patient_Id']))
